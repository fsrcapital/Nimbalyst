 ...
 * QR Pairing Modal
 *
 * Shows a QR code for either the native iOS app or the browser-based Web App.
 * Native pairing retains the upstream v5 payload; Web App pairing additionally
 * includes the narrowly scoped desktop-gateway credential.
 */
export function QRPairingModal({ isOpen, onClose, serverUrl, pairingTarget, preventSleepMode, onPreventSleepModeChange }: QRPairingModalProps) {
  ...
  const generateQR = useCallback(async () => {
    ...
    try {
      const payload = await window.electronAPI.credentials.generateQRPayload(effectiveUrl, pairingTarget);
      ...
      const dataUrl = await QRCode.toDataURL(deepLinkUrl, {
        width: pairingTarget === 'web' ? 340 : 280,
        margin: pairingTarget === 'web' ? 3 : 2,
        color: {
          dark: '#000000',
          light: '#ffffff',
        },
        errorCorrectionLevel: pairingTarget === 'web' ? 'L' : 'M',
      });
    ...
    >
      ...
      >
        <div className="qr-modal-header flex items-center justify-between px-5 py-4 border-b border-nim sticky top-0 bg-nim z-10">
          <h2 className="qr-modal-title text-lg font-semibold text-nim m-0">
            {pairingTarget === 'web' ? 'Pair Web App' : 'Pair iPhone'}
          </h2>
          <button
          ...
          {error ? (
            ...
            <>
              ...
              <div className="qr-instructions text-sm text-nim-muted space-y-1 mb-4">
                {pairingTarget === 'web' ? (
                  <>
                    <p className="qr-step">1. Open the Nimbalyst Command Center Web App</p>
                    <p className="qr-step">2. Tap &quot;Scan desktop QR code&quot;</p>
                    <p className="qr-step">3. Point your camera at this QR code</p>
                    <p className="qr-step">4. Enter your private Tailscale gateway address</p>
                  </>
                ) : (
                  <>
                    <p className="qr-step">1. Open Nimbalyst on your iPhone</p>
              ...
              <div className="qr-info mt-3 p-3 bg-green-500/10 border border-green-500/20 rounded-lg text-[13px] text-nim-muted">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-semibold text-green-500">
                    {pairingTarget === 'web' ? 'Private Direct Connection' : 'End-to-End Encrypted'}
                  </span>
                </div>
                <p className="m-0">
                  {pairingTarget === 'web'
                    ? 'This QR code transfers your encryption key and desktop gateway credential directly to your device. Neither is sent to Nimbalyst.'
                ...
                <div className="mt-3 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                  <div className="flex items-start gap-2.5">
                    <div className="flex-1">
                      <span className="text-[13px] font-medium text-nim">
                        {pairingTarget === 'web' ? 'Prevent sleep while remotely accessible' : 'Prevent sleep while syncing'}
                      </span>
              ...
              <div className="qr-warning flex items-center gap-2 mt-3 p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg text-xs text-amber-600">
                ...
                <span>
                  {pairingTarget === 'web'
                    ? 'Only scan with your own device. This shares your private desktop credential.'
                    : 'Only scan with your own device. This shares your encryption key.'}
                </span>
              </div>
              ...
              {qrPayload && (
                <div className="qr-dev-copy">
                  ...
                  <p className="mt-2 text-[11px] text-nim-faint text-center">
                    Can&apos;t scan? Paste this into the {pairingTarget === 'web' ? 'Web App' : 'mobile app'}&apos;s Manual Setup
                  </p>
                </div>
              )}
            </>
