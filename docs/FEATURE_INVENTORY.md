...
- Personal workflow metadata — notes, next action, waiting-on context, and explicit attention reasons — shown in session views and synced across paired devices; cache-warming controls preserve context for work that will resume later
...
- Installable mobile command-center PWA paired to a desktop workspace
- Browse workspace session hierarchies and transcripts, see live execution/subagent state, and filter sessions that need attention
- Start desktop-backed Claude or Codex sessions (including isolated worktrees), send follow-up prompts, cancel running work, and answer permission, plan-approval, and question prompts
- Edit session phase and personal workflow metadata (notes, next action, waiting-on context, and attention reasons), with web push notifications for waiting sessions
